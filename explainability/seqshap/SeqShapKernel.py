...
class SeqShapKernel(KernelExplainer):
    ...
    def __call__(self, X):
        ...
        seg = SeqShapSegmentation(lambda x: np.mean(x, axis=0), self.seq_num, self.dataset_name, True)
        ...
        seg = SeqShapSegmentation(lambda x: x, self.seq_num, self.dataset_name, False)
        ...
        visualize_phi_seq(self.phi_seq, f"plots/{self.dataset_name}/SeqSHAP/Sequence_{self.seq_num}", "phi_seq.html")
